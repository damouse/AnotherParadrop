from paradrop.chute import restart
from mock import patch, MagicMock


@patch('paradrop.chute.chutestorage.ChuteStorage')
...
@patch('paradrop.chute.restart.timeint')
@patch('paradrop.chute.restart.log')
@patch('paradrop.chute.restart.time')
@patch('paradrop.chute.restart.waitSystemUp')
@patch('paradrop.chute.restart.reclaimNetworkResources')
@patch('paradrop.chute.restart.settings')
@patch('paradrop.chute.chutestorage.ChuteStorage')
def test_reloadChutes(mockStore, mockSettings, mResources, mWait, mTime, mOut, mTimeint):
    ...
    assert dict(updateClass='CHUTE', updateType='stop', name=ch1.name,
                tok=mTimeint.return_value, func=restart.updateStatus, warning=restart.FAILURE_WARNING) in ret
    assert dict(updateClass='CHUTE', updateType='restart', name=ch3.name,
                tok=mTimeint.return_value, func=restart.updateStatus) in ret
