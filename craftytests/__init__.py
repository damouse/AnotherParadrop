...
class CommandError(Exception):
    ...
    def __str__(self):
        return 'Error running command {!r} in {!r}. {}'.format(self.command, self.working_dir, self.message)
...
class SnapsTestCase(testtools.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        relative_path = os.path.relpath(os.path.dirname(inspect.getfile(self.__class__)), os.path.dirname(__file__))
        self.src_dir = os.path.join(*re.findall('(.*?)_tests/?(.*)', relative_path)[0])
        super(SnapsTestCase, self).__init__(*args, **kwargs)
    ...
    def setUp(self):
        ...
        if filter_:
            if not re.match(filter_, self.snap_content_dir):
                self.skipTest('{} does not match the filter {}'.format(self.snap_content_dir, filter_))

        logger.info('Testing {}'.format(self.snap_content_dir))
        super(SnapsTestCase, self).setUp()

        self.snapcraft_command = 'snapcraft'
        self.useFixture(fixtures.EnvironmentVariable('SNAPCRAFT_SETUP_PROXIES', '1'))
        ...
        if not config.get('skip-install', False):
            ...
            if not ip:
                ...
                self.snappy_testbed = testbed.SshTestbed(ip, port, 'ubuntu')
    ...
    def _set_up_qemu_testbed(self):
        ...
        snappy_testbed = testbed.QemuTestbed(snappy_image, '8022', 'ubuntu', private_key, _KVM_REDIRECT_PORTS)
    ...
    def _snap(self, project_dir):
        ...
        self._run_command(command, project_dir, expect='Snapped .*\.snap', timeout=1200)
    ...
    def _run_command(self, command, working_dir, expect=pexpect.EOF, timeout=30):
        ...
        try:
            ...
            raise CommandError(command, working_dir, 'Expected output {!r} not found.'.format(expect))
        finally:
            process.close()
            if process.exitstatus:
                self._add_output_detail(process.before)
                raise CommandError(command, working_dir, 'Exit status: {!r}.'.format(process.exitstatus))
    ...
    def install_snap(self, snap_content_dir, snap_name, version, devmode=False):
        ...
        if not config.get('skip-install', False):
            snap_file_name = '{}_{}_*.snap'.format(snap_name, version)
            snap_local_glob_path = os.path.join(self.src_dir, snap_content_dir, snap_file_name)
            snap_local_path = glob.glob(snap_local_glob_path)[0]
            self.snappy_testbed.copy_file(snap_local_path, '/home/ubuntu')
            snap_path_in_testbed = os.path.join('/home/ubuntu/', snap_file_name)

            # Remove the snap file from the testbed.
            self.addCleanup(self.snappy_testbed.run_command, ['rm', snap_path_in_testbed])
            ...
            self.addCleanup(self.snappy_testbed.run_command, ['sudo', 'snap', 'remove', snap_name])
    ...
    def assert_service_running(self, snap, service):
        if not config.get('skip-install', False):
            output = self.run_command_in_snappy_testbed(['systemctl', '--no-pager', 'status', 'snap.{}.{}'.format(snap, service)])
